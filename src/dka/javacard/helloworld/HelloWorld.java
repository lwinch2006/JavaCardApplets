...
public class HelloWorld extends Applet implements ExtendedLength {
    ...
    private final SharedCardService _cardService;
    private final SampleService _sampleService;
    private final SharedCommandChainingService _commandChainingService;
    ...
    protected HelloWorld() {
        ...
        _cardService = new SharedCardService();
        _sampleService = new SampleService();
        _commandChainingService = new SharedCommandChainingService(MAX_COMMAND_CHAINING_BUFFER_SIZE);
    }
    ...
    public void process(APDU apdu) {
        ...
        if (CLA != SharedApduConstants.HW_CLA && CLA != SharedApduConstants.HW_CHAIN_CLA) {
        ...
        switch (INS) {
            ...
            case ApduConstants.HW_GET_RESPONSE_INS:
                SharedApduUtils.getResponse(apdu);
                return;
            ...
            case SharedApduConstants.HW_GET_APDU_BUFFER_LENGTH_INS:
                _cardService.getAPDUBufferLength(apdu);
                return;

            case SharedApduConstants.HW_GET_INBLOCKSIZE_INS:
                _cardService.getInBlockSize(apdu);
                return;

            case SharedApduConstants.HW_GET_OUTBLOCKSIZE_INS:
                _cardService.getOutBlockSize(apdu);
                return;

            case SharedApduConstants.HW_GET_AVAILABLEPERMANENTMEMORY_INS:
                _cardService.getAvailablePermanentMemory(apdu);
                return;

            case SharedApduConstants.HW_GET_AVAILABLETRANSIENTMEMORY_ONRESET_INS:
                _cardService.getAvailableTransientMemoryOnReset(apdu);
                return;

            case SharedApduConstants.HW_GET_AVAILABLETRANSIENTMEMORY_ONDESELECT_INS:
                _cardService.getAvailableTransientMemoryOnDeselect(apdu);
                return;

            case SharedApduConstants.HW_GET_JCAPIVERSION_INS:
